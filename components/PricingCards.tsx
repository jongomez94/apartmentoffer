...
export default function PricingCards({ pricing }: { pricing: PricingContent }) {
  return (
      ...
      <div className="mx-auto max-w-5xl px-6">
        ...
        <div className="grid gap-8 md:grid-cols-2">
          <motion.article
            ...
            viewport={{ once: true }}
          >
            <h3 className="font-serif text-2xl font-medium text-stone-900">{pricing.option1.name}</h3>
            <div className="mt-4 flex items-baseline gap-1">
              <span className="font-serif text-4xl font-medium text-stone-900">{pricing.option1.price}</span>
              <span className="font-sans text-stone-500">{pricing.option1.period}</span>
            </div>
            <ul className="mt-6 flex-1 space-y-3">
              {pricing.option1.features.map((feature, i) => (
                <li key={i} className="flex items-center gap-2 font-sans text-stone-600">
          ...
          >
            ...
            <h3 className="font-serif text-2xl font-medium text-stone-900">{pricing.option2.name}</h3>
            <div className="mt-4 flex items-baseline gap-1">
              <span className="font-serif text-4xl font-medium text-sage">{pricing.option2.price}</span>
              <span className="font-sans text-stone-500">{pricing.option2.period}</span>
            </div>
            <ul className="mt-6 flex-1 space-y-3">
